...
const FilesController = class {
  ...
  static async getIndex(req, res) {
    ...
    if (folder === null && parentId !== '0') return res.json([]);
  ...
  static async putPublish(req, res) {
    ...
    return res.status(200).json({ id: file._id, ...pfile });
  }
  ...
  static async getFile(req, res) {
    ...
    if (!file.isPublic) {
      const userId = await redisClient.get(`auth_${req.headers['x-token']}`);
      if (userId === null || userId !== file.userId.toString()) {
    ...
    if (file.type === 'file') return res.send(content);
